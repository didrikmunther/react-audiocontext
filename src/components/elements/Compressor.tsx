...
export const Compressor = ({ audio, input, out, initial }: CompressorProps) => {
    const [compressor] = useState<DynamicsCompressorNode>(new DynamicsCompressorNode(audio));

    const [threshold, thresholdKnob] = useKnob(initial.release ?? -50, { min: -100, max: 0 });
    const [knee, kneeKnob] = useKnob(initial.release ?? 40, { min: 0, max: 40 });
    const [ratio, ratioKnob] = useKnob(initial.release ?? 12, { min: 1, max: 20 });
    const [attack, attackKnob] = useKnob(initial.release ?? 0, { max: 1 });
